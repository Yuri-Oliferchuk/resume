...
api1_0.post("/:lang/info", jwtAdminTokenCheckMiddelware, async (req, res) => {
  ...
  try {
    const sql =
      "UPDATE info SET name=$1, profession=$2, text=$3, contacts=$4 WHERE lang=$5;";
...
api1_0.post("/auth/login", (req, res, next) => {
  passport.authenticate("local", { failureFlash: true }, (err, user, info) => {
    ...
    if (!user)
      return res
        .status(401)
        .json({ message: info.message, statusCode: info.statusCode });
...
api1_0.post("/auth/signup", async (req, res, next) => {
  ...
  if (email && username && password) {
    ...
    const getAllUsers =
      "SELECT username FROM users_auth WHERE username LIKE $1;";
    const result = await pool.query(getAllUsers, [username]);

    if (!result.rows[0]) {
      const sql =
        "INSERT INTO users_auth (username, password, email, salt, superuser) VALUES ($1, $2, $3, $4, $5);";
      await pool.query(sql, [username, hashedPassword, email, salt, false]);
    } else {
      return res
        .status(400)
        .json({ message: "Username already exist", statusCode: 1 });
    }
